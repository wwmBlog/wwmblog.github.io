...
module Jekyll
  ...
  class CategoryList < Liquid::Tag
    ...
    def render(context)
      ...
      categories = context.registers[:site].categories
      categories.keys.sort_by{ |str| str.downcase }.each do |category|
        url = category_dir + category.gsub(/_/, '-').gsub(/-{2,}/, '-').downcase
        html << "<li><a class='li_cat' href='#{url}'>#{category.capitalize}"
        if @opts['counter']
          html << "</a><span class='count'>#{categories[category].count}</span>"
        end
        html << "</li>"
      end
      html
